...
RUN curl -L "https://github.com/bbusse/gentoo-config/archive/refs/heads/${GENTOO_CONFIG_REF}.tar.gz" | \
    ...
    cp /tmp/gentoo-config/package.use.hwaccel-amd64 /etc/portage/ && \
    cp -r /tmp/gentoo-config/env /etc/portage/env && \
    cp /tmp/gentoo-config/package.env.arm64 /etc/portage/ && \
